...
Im Fernstudium muss man seine Zeit weitgehend selbst einteilen. Die hochschuleigenen Portale zeigen zwar Noten, aber keine laufende Zeitplanung mit Verzögerungen und Korrekturphasen. Genau da setzt \textit{Uni Speedrun} an: ein einfaches, lokales Terminal-Tool (TUI).

Ziel von Phase 3 war die vollständige Implementierung des in Phase 1 konzipierten und in Phase 2 architektonisch modellierten Systems. Der Prototyp liefert eine automatisierte Fortschritts- und Zeitprognose, unterstützt einen praxisnahen Modul-Lebenszyklus, persistiert alle Daten in einer lokalen SQLite-Datenbank und setzt die Schichtenarchitektur aus Phase 2 mit dem Feedback von damals um.

\section{Methodik, Schichtenarchitektur und technische Umsetzung}
Ich habe mich bei der Umsetzung in Python 3.10+ an den SOLID-Prinzipien orientiert. Als Reaktion auf das Feedback der Portfoliophase 2 wurde das Fachmodell erweitert sowie eine dedizierte Controller-Schicht zur klaren Trennung von UI und Persistenz eingeführt:
...
\section{Fazit}
Mit \textit{Uni Speedrun} habe ich einen funktionierenden Prototyp für die Studienplanung gebaut, der zeigt, wie objektorientierte Konzepte, Tests und eine saubere Architektur in der Praxis zusammenspielen. Die im Konzept festgelegten Kernfunktionen wurden umgesetzt und anhand der Tests überprüft.
